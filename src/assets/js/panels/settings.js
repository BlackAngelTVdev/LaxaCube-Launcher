...
class Settings {
  ...
  async initGameDir() {
    document
      .querySelector(".open-game-dir")
      .addEventListener("click", async () => {
        const path = await appdata();
  ...
  // 2. Gestion de la console
  async initConsole() {
    let configClient = await this.db.readData("configClient");
    let consoleBtn = document.querySelector(".console-toggle-btn");
    let isConsoleActive = configClient?.launcher_config?.showConsole || false;

    if (isConsoleActive) consoleBtn.classList.add("active-close");

    consoleBtn.addEventListener("click", async () => {
      let config = await this.db.readData("configClient");
      isConsoleActive = !isConsoleActive;

      consoleBtn.classList.toggle("active-close");
      config.launcher_config.showConsole = isConsoleActive;
  ...
  async initRepair() {
    document
      ...
      .addEventListener("click", async () => {
        if (
          confirm(
            "Voulez-vous vraiment réparer le jeu ? Cela supprimera les fichiers temporaires pour forcer une mise à jour."
          )
        ) {
          const path = await appdata();
          const fs = require("fs");
          const gamePath = require("path").join(
            path,
            process.platform === "darwin"
              ? this.config.dataDirectory
              : `.${this.config.dataDirectory}`
          ...
          // On supprime par exemple le dossier runtime ou assets pour forcer le retéléchargement
          const runtimePath = require("path").join(gamePath, "runtime");
          if (fs.existsSync(runtimePath)) {
            fs.rmSync(runtimePath, { recursive: true, force: true });
            alert(
              "Réparation effectuée. Le launcher va retélécharger les composants nécessaires."
            );
            window.location.reload();
          } else {
            alert("Tout semble déjà propre !");
          }
        }
      });
  }
